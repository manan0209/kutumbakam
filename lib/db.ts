...
export type ResourceNeed = {
  ...
  title: string;
  description: string;
  category: "medicine" | "food" | "shelter" | "clothing" | "water" | "transport" | "other";
  quantity: number;
  unit?: string;
...
export async function getActivePortals() {
  ...
  return querySnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() } as DisasterPortal));
}

export async function updatePortal(portalId: string, portalData: Partial<DisasterPortal>) {
  const portalRef = doc(db, "disasterPortals", portalId);
...
export async function createResourceNeed(resourceData: Omit<ResourceNeed, "id" | "createdAt">) {
...
export async function getResourceNeeds(portalId: string) {
  ...
  const querySnapshot = await getDocs(q);
  return querySnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() } as ResourceNeed));
}
...
export async function updateResourceNeed(resourceId: string, resourceData: Partial<ResourceNeed>) {
...
// Volunteer functions
export async function registerVolunteer(volunteerData: Omit<Volunteer, "id" | "registeredAt">) {
...
// Update functions
export async function createUpdate(updateData: Omit<Update, "id" | "createdAt">) {
